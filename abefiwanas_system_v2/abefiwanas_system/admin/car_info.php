...
if(!empty($_POST))
{
    ...
    values
    ('$numPlate','$carName','carType','$color','$yearManufac','$initialPrice','$model_ID','idimg')";
...
$arahan_sql_cari="select* from car,model where car.numPlate not in (select numPlate from purchase) and car.model_ID=model.model_ID";
...
<table class="w3-table-all" id='saiz' border='1'>
    ...
    <tr>
        ...
        <form action='' method='POST'>
            ...
            <td>
                <!----------------------------------------------------------------------------------------------------------------------------------------------->
                <!-- menghasilkan drop down yg dinamik (mengambil data dari jadual) -->
                <select name='idimg' required>
                    <option disabled selected value>Image</option>
                    <?PHP
                
                # arahan mencari data dari jadual gambar
                $arahan_sql_gambar= "SELECT* from images";
                # melaksanakan arahan mencari tersebut
                $laksana_sql_gambar=mysqli_query($condb,$arahan_sql_gambar);
                # pembolehubah $rekod_gmbr mengambil data baris demi baris 
                while($rekod_gmbr=mysqli_fetch_array($laksana_sql_gambar))
                {   
                    # memaparkan nilai pemboleh ubah $rekod_gmbr['image'] dalam bentuk dropdown list
                    echo"<option value='".$rekod_gmbr['idimg']."'> ".$rekod_gmbr['idimg']
                    ."</option>";
                }
                ?>
                </select>
    ...
    while ($rekod=mysqli_fetch_array($laksana_sql_cari))
    {
        ...
        <tr>
            ...
            <td><a href='hapus.php?jadual=kereta&medan_kp=noplat&kp=".$rekod['numPlate']."' onClick=\"return confirm('Confirm to delete data?')\" >Delete</a></td>
        </tr>";
    }
    ?>
</table>
